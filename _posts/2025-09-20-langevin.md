...
description: "Connecting classical mechanics to statistical mechanics via the Langevin equation: giving a pendulum a temperature through stochastic forcing."
---
> Phase space image from https://tikz.net/dynamics_phaseportrait/.
...
A very standard question in introductory statistical mechanics courses is:
  ...
  *(a) Calculate the partition function $Z(\beta)$*.

  *(b) Show that the average energy for the oscillator is $3k_BT$.*
...
$$\langle A \rangle_t \equiv \frac{1}{2t}\int_{-t}^t A(s) ds.$$
...
i.e. it picks up the $H=E$ hypersurface. This is the so-called *microcanonical ensemble*. There is no unique, *a priori* notion of temperature (cf [here](https://en.wikipedia.org/wiki/Microcanonical_ensemble#Thermodynamic_quantities)) for this ensemble.
...
Importantly, notice that the **mechanism** by which our system is kept in equilibrium with the environment is assumed irrelevant. Whatever it is, its role is to keep the average energy at $3 k_B T$, and analogously for higher moments.
...
1. It truly behaves like a particle connected to a heat bath
2. It reaches temperature $T$ as $t\to\infty$.
...
where we assumed that $\mathbb E[\eta(s) \, \vert \, v_0 ] = \mathbb E[\eta(s)]$, which is valid, for instance, if the noise term is independent of $v_0$ (which we will assume it is). The rigorous proof of why this formal operation works requires an understanding of the Ito integral -- see [Shreve 2004].
...
Hence, the velocity autocorrelation decays exponentially when the dissipation force is white noise. Intuitively, it means that the particle's velocity loses all correlation with its initial one after an interval of the order $\sim 1/\gamma$.
...
$$\partial_x (vp) = v  \partial_xp,\quad \partial_v (p(\partial _xU(x)))=U'(x) \, \partial_vp$$

obtaining the **Klein-Kramers equation** (also known as the **Kramers-Chandrasekhar** equation):

$$\boxed{\frac{\partial p}{\partial t}+v \partial_x p=\frac 1m U'(x) \partial_vp+\frac{\gamma}{m} \partial_v \left[ pv+\frac{k_B T}{m} \partial_vp\right]}$$

Notice that this equation is of the form

$$\frac{Dp}{Dt} -\frac 1mU'(x) \, \partial_vp = \partial_v  j,\quad j\equiv\frac{\gamma}{m}\left[ pv+\frac{k_B T}{m} \partial_vp\right]$$
...
$$-v  \partial_x p_\infty+\frac 1m U'(x)\,  \partial_vp_\infty+\frac{\gamma}{m} \partial_v \left[ p_\infty v+\frac{k_B T}{m} \partial_vp_\infty\right]=0.$$
...
$$\partial_v p_\infty=- p_\infty v \frac{m}{k_BT}\quad\text{ and } \quad v \partial_xp_\infty=\frac 1m U'(x)\,\partial_vp_\infty.$$
...
$$\partial_x p_\infty = \frac 1m \partial_x U\,\left(-p_\infty \frac{m}{k_BT} \right) \implies\partial_x p_\infty=-\frac{p_\infty}{k_BT} U'(x)$$
...
$$
m \frac{dv(t)}{dt}= \gamma m v(t)+  F(t)
$$
...
is the sum of $N$ similar Hamiltonians $H_j$. Upon exponentiating, we will have the product of $N$ exponentials:
...
$$p_\infty \propto e^{H'}/k_BT$$

for some Hamiltonian $H'$, then our job is done.

Unfortunately, for general colored (i.e. non-white) noise, there is no natural mapping between Langevin and Fokker-Planck equations([Hänggi (1997)](https://www.physik.uni-augsburg.de/theo1/hanggi/Papers/201.pdf) eq 17). This means that, if we look up results in literature, they will likely:
* Be valid for specific memory kernels $K$
* Be valid for specific subcases of the generalized Langevin equation

Below, we provide a proof of the claim in [Ayaz et al (2022)](https://refubium.fu-berlin.de/bitstream/handle/fub188/37874/Self-consistent%20Markovian%20embedding%20of%20generalized%20Langevin%20equat.pdf?sequence=1) , Equations (3.1)-(3.4), that we can convert a generalized Langevin equation with an *exponential kernel* into a standard Langevin equation, for which there *is* a Fokker-Planck equation!
...
$$\dot v(t)=-\frac 1m U'(x) -\int_{0}^t \Gamma_0 e^{-(t-t')/\tau}v(t')dt'+ \frac 1m R(t)\tag{$*$}$$

with noise satisfying 

$$\langle R(t)\rangle = 0,\qquad \langle R(t)R( t')\rangle= mk_BT \Gamma_0 e^{\vert t-t'\vert /\tau}$$

Initial conditions are $x(0) = x_0$, $v(0) = v_0$ as usual.
...
where $\eta$ is white noise as usual:
...
It is trivial to see that $\langle G(t)\rangle = 0$ from the fact that $\eta$ is white noise. The calculation of the variance is completely analogous to the one we did for the *velocity* in the white-noise Langevin equation, with the caveat that for a term $e^{-(t+t')/\tau}$, we will need to consider $t,t \gg 1/\tau$.
...
* [Kubo's 1966 text](http://www-f1.ijs.si/~ramsak/km1/kubo.pdf) on the generalized Langevin equation and a general fluctuation-dissipation theorem, constructed from the Fourier-Laplace transform
